...
stretchingRouter.get('/', async (req, res, next) => {
    try {
        ...
        const allSessions = await Stretching.find({ date: { $gte: today } })
            .populate('users.data')
            .sort({ date: 1 });
...
stretchingRouter.put('/:id', async (req, res, next) => {
    try {
        ...
        const joinCriteriaPassed =
            ...
            stretchingAppointment.users.filter(participant => participant.data._id.toString() === user._id.toString())
                .length === 0;
        ...
        if (joinCriteriaPassed) {
            ...
            stretchingAppointment.users = stretchingAppointment.users.filter(
                participant => participant.data._id.toString() !== user._id.toString(),
            );

            const saved = await stretchingAppointment.save();
            await saved.populate('users.data').execPopulate();

            user.stretchingSessions = user.stretchingSessions.filter(
                stretch_session_id => stretch_session_id.toString() !== stretchingAppointment._id.toString(),
            );
            await user.save();
