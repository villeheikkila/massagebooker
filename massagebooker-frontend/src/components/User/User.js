...
const User = ({ id, name, email, number, admin, avatarUrl, mobile }) => {
        ...
        return (
            <div onClick={() => toggleVisibility()}>
                ...
                <div style={{ display: visibility }}>
                    ...
                    Role: {role} <br />
                    <button className="makeAdminButton" onClick={() => toggleAdmin()}>
                        {adminButtonToggleText}
                    </button>{' '}
                    <br />
                    <button className="removeUserButton" onClick={() => removeUser(id)}>
                        Remove
                    </button>
                </div>
            </div>
    ...
    return (
        <tr>
            <td>
                {avatarUrl ? <img className="dashboard_profile_image" src={avatarUrl} alt="profile pic" /> : 'avatar'}
            </td>
            <td>{name} </td>
            <td>
                <a id="mail_link" href={`mailto:${email}`}>
                    {email}
                </a>
            </td>
            <td>{role}</td>
            {number ? (
                <td>
                    <a id="phone_link" href={`tel:${number}`}>
                        {number}
                    </a>
                </td>
            ) : (
